...
class Schedule extends Component {
  ...
  filterEvents() {
    ...
    this.state.data.forEach((event) => {
      const startDate = new Date(event[this.state.columnNames[6]]);
      const endDate = new Date(event[this.state.columnNames[11]]);

      if (
        this.state.date >= startDate.getTime() &&
        this.state.date <= endDate.getTime()
      ) {
        current.push(event);
  ...
  getEventData(events, flag) {
    ...
    events.forEach((item) => {
      const title = item[this.state.columnNames[1]];
      const startDate = new Date(item[this.state.columnNames[6]]);
      const endDate = new Date(item[this.state.columnNames[11]]);

      const text =
        (flag ? "Now" : moment(startDate).format("LT")) +
        " - " +
    ...
    if (this.state.data.length === 0) {
      ...
      contents = (
        <List
          ...
          dataSource={this.getEventData(res[1], true).concat(
            this.getEventData(res[2], false)
          )}
          renderItem={(item) => (
            <List.Item>
              <List.Item.Meta
        ...
        <Meta
          title={
            <div>
              <img
                src={scheduleLogo}
                style={{ height: "12%", width: "12%", float: "left" }}
              />
              <h1
                style={{
                  color: "#1A2E33",
